...
class ${className}Controller {
    ...
    def index(Integer max) {
        ...
        if (request.xhr) {
            def result = [list: ${propertyName}Service.list(params), total: ${propertyName}Service.count()]
    ...
    def show(Long id) {
        if (request.xhr) {
            def result = [item: ${propertyName}Service.get(id)]
            render result as JSON
        } else {
    ...
    def delete(Long id) {
        ...
        if (request.xhr) {
            def result = [message: message(code: 'default.deleted.message', args: [message(code: '${propertyName}.label', default: '${className}'), id])]
            render result as JSON
        } else {
            request.withFormat {
                form multipartForm {
